...
class InventoryListenerTests {
    ...
    @Test
    void orderPlacedPublishesReservedEventWhenReservationSucceeds() {
        ...
        inventoryListener.onOrderPlaced(envelope("order.v1.placed", order, "trace-123"), "trace-123");
    ...
    @Test
    void orderConfirmedConsumesReservedInventory() {
        inventoryListener.onOrderConfirmed(envelope("order.v1.confirmed", new OrderConfirmed("77"), "trace-456"));

        verify(inventoryService).confirm("77");
    }

    @Test
    void orderCancelledReleasesReservation() {
        inventoryListener.onOrderCancelled(envelope("order.v1.cancelled", new OrderCancelled("88", "payment_failed"), "trace-789"));

        verify(inventoryService).release("88");
    }

    @Test
    void orderPlacedPublishesRejectedEventWhenReservationFails() {
        ...
        inventoryListener.onOrderPlaced(envelope("order.v1.placed", order, "trace-999"), "trace-999");
